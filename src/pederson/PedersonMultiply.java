...
public class PedersonMultiply {
    ...
    public Element[] zkpProverStep1() {
        ...
        result[0] = PedersonShare.genData_pp.pow(d).
                mul(PedersonShare.genVerif_pp.pow(s)); //M
        result[1] = PedersonShare.genData_pp.pow(x).
                mul(PedersonShare.genVerif_pp.pow(s1)); //M1
        result[2] = bShare.computeMac(bShare.index).pow(x).
                mul(PedersonShare.genVerif_pp.pow(s2));// M2
    ...
    public BigInteger[] zkpProverStep2(BigInteger e) {
        ...
        result[0] = d. add(e.multiply(beta)). mod(modQ); //y
        result[1] = s. add(e.multiply(sigma)).mod(modQ); //w
        result[2] = x. add(e.multiply(alpha)).mod(modQ); //z
        result[3] = s1.add(e.multiply(rho)).  mod(modQ); //w1
        result[4] = s2.add(e.multiply(tau.subtract(sigma.multiply(alpha)
                .mod(modQ))
        ).mod(modQ)).mod(modQ); //w2
