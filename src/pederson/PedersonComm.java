...
public class PedersonComm {
     ...
     * long as one of them is honest. The random number is either 0 or 1
     *
     * @param threshold The reveal threshold of the shares generated.
     */
    public static PedersonShare RAN2(int threshold, Channel[] channels) throws IOException, CheatAttemptException {
        // Generate a shares of a random number
        BigInteger rNum = new BigInteger(BigInteger.ONE.bitLength(), random);
        return shareNumber(rNum,threshold,channels);
    }


    /**
     * Takes two shared values and return one paired value.
     *
     *
     */
    public static BigInteger convergeShares(String uidPrf,PedersonShare metaDataShares, Channel[] channels) throws IOException, CheatAttemptException {

        BigInteger uid = new BigInteger(uidPrf);
        BigInteger metaData = new BigInteger(plaintextBilinearExponentiate(metaDataShares,channels).toBytes());

        BigInteger sum = uid.add(metaData);
        BigInteger sum1 = sum.add(new BigInteger("1"));

        BigInteger mul = sum.multiply(sum1);
        BigInteger result = mul.divide(new BigInteger("2"));
        return result;
    }
    /**
    * Takes exponentiated shares and convert it into a bigInteger of the PRF macs
    */
    public static boolean verifyUid(byte[][] expShareBytes , int serverID , Channel[] channels, BigInteger Uid) throws IOException, CheatAttemptException {
    	for (Channel channel : channels)
            if (channel != null)
                channel.send((Serializable)expShareBytes);
	Element[][] expShares = new Element[channels.length][];
	expShares[serverID] = new Element[expShareBytes.length];
	for (int j = 0; j < expShareBytes.length; ++j) {
                    if (j < 2)
                        expShares[serverID][j] = PedersonShare.groupG1.newOneElement();
                    else
                        expShares[serverID][j] = PedersonShare.group.newOneElement();
                    expShares[serverID][j].setFromBytes(expShareBytes[j]);
        }
        for ( int channel = 0 ; channel < channels.length  ; channel++) {
            if (channel == serverID)
                continue; //Ignore. Probably it is this player's position.
            try {
            	byte[][] expShareBytesOthers = (byte[][])channels[channel].receive();
                expShares[channel] = new Element[expShareBytesOthers.length];
                for (int j = 0; j < expShareBytesOthers.length; ++j) {
                    if (j < 2)
                        expShares[channel][j] = PedersonShare.groupG1.newOneElement();
                    else
                        expShares[channel][j] = PedersonShare.group.newOneElement();
                    expShares[channel][j].setFromBytes(expShareBytesOthers[j]);
                }
            }
            catch (IOException|ClassNotFoundException e) {
                continue;
        ...
        BigInteger UidCom = new BigInteger(plaintextBilinearExponentiateRecv(expShares, channels.length / 2,channels.length).toBytes());
        if(UidCom.compareTo(Uid) == 0 ) return true ;
        return false ;
        
    }
    
    public static PedersonShare piCompare(int val1 , PedersonShare val2 , Channel[] channels) throws IOException, CheatAttemptException {
    	BigInteger a = BigInteger.valueOf(val1);
        BigInteger b = combineShares(val2,channels);
        if( a.compareTo(b) >= 0) return shareNumber(BigInteger.ONE, channels.length/2, channels);
        else return shareNumber(BigInteger.ZERO, channels.length/2, channels);
    }
    ...
    /** Splits the shares of a given value
    *
    */
    public static PedersonShare[] getValueShares(BigInteger val ,int threshold, Channel[] channels) throws IOException, CheatAttemptException {
    	String bits = val.toString(2) ;
    	int l = bits.length() ;
    	PedersonShare[] shares = new PedersonShare[l] ;
    	for(int i = 0 ; i < l ; i++)  shares[l-1-i] = shareNumber(new BigInteger(String.valueOf(bits.charAt(i))), threshold, channels) ;
        return shares;
    }
    
    /**
     * Generates shares of a given number that no party knows about as
     * long as one of them is honest. The number is uniformly
     * distributed in 0 to modQ - 1.
     *
     * @param threshold The reveal threshold of the shares generated.
     */
    public static PedersonShare shareNumber(BigInteger value, int threshold, Channel[] channels) throws IOException, CheatAttemptException {
    
        PedersonShare[] shares = PedersonShare.shareValue(value, threshold, channels.length);
