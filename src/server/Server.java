		    ...
		    // UID check
                    ...
                    for(int mx = 5  ; mx >= 0 ; mx--){
                    	...
                    	while(allegationsResultSet.next()) {//find revealableSetCount
                    		...
                    		PedersonShare test = one;
                    		for(int i = 0 ; i < metaDataShare.length ; i++) {
                    			test = (PedersonComm.multiply(test,one.subtract(Pid_i[i].subtract(metaDataShare[i])),channels));
                    		}                    		
                    		PedersonShare chk4 = flag ;
                    		if(PedersonComm.combineShares(test,channels).compareTo(BigInteger.ONE) == 0 ) chk4 = one.subtract(flag);
                    		System.out.println("The test value is " + PedersonComm.combineShares(test,channels));
